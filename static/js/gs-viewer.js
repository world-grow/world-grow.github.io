...
async function main() {
  ...
  window.loadModel = async function (modelUrl) {
    try {
      ...
      if (isPly) {
        worker.postMessage({ ply: buf, save: false }, [buf]);
      } else {
        const vertexCount = Math.floor(buf.byteLength / (3*4 + 3*4 + 4 + 4));
        worker.postMessage({ buffer: buf, vertexCount: vertexCount }, [buf]);
      }
      console.log("loadModel: queued", modelUrl);
    } catch (err) {
  ...
  window.selectModel = async function (element) {
    ...
    loadModel(element.getAttribute("value"));
  }
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      const zoom = (-10 * (e.deltaY * scale)) / innerHeight;
      let inv = invert4(viewMatrix);
      inv = translate4(inv, 0, 0, zoom);
